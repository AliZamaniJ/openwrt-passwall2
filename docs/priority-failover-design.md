...
Every failed endpoint probe records the node ID, sanitized URL, curl exit status, HTTP status, connect time, application-connect time, and total time in syslog with the `probe-endpoint-failed` reason. With the SOCKS proxy, `time_connect` ends when curl connects to the local proxy, while `time_appconnect` is cumulative from probe start through proxy negotiation, proxy-side DNS and outbound connection establishment, and—for HTTPS—the remote TLS handshake. It is therefore not labeled as TLS-only duration. URL userinfo through the final raw `@` in the authority, query parameters, fragments, and line-breaking characters are removed from log output so even malformed custom probe credentials cannot leak through diagnostics; `@` characters after the first path slash are preserved. A confirmed active-node failure is recorded as `node-failed` when another candidate answers the same probes. If no candidate answers, the supervisor checks every IPv4 and IPv6 default route, including every nexthop in an ECMP route, and treats the WAN as available when any route candidate has a usable device whose carrier is not explicitly down. No default route, no route device, or an explicit carrier-down result across every route candidate is recorded as `wan-down`; otherwise the failure is recorded as `probe-endpoint-failed` and follows the configured all-down fallback policy. A family-aware gateway ping is diagnostic only because an unanswered ICMP request does not prove that the WAN is unavailable and must never suppress the all-down fallback.
