...
The primary probe is `https://www.gstatic.com/generate_204`. If it fails, `https://cp.cloudflare.com/generate_204` confirms the failure. Custom URLs are also supported; any final HTTP 2xx response is healthy. A node is unhealthy only when both endpoints fail. The failed-health-cycle threshold accepts values from two through five; values outside that range fall back to the default of two. By default, the second failed health cycle cannot complete the failure decision until at least ten seconds after the first failed cycle, which filters short correlated network stalls without requiring a third cycle. Failure debounce, backup dwell, and recovery intervals use system uptime from `/proc/uptime`, so NTP or manual wall-clock corrections cannot shorten or stall these decisions; wall-clock timestamps remain available in state and logs for diagnostics. The live `failure_count` and `first_failure_at` fields remain scoped to the current node and reset after a successful transition. The separate `last_failed_id`, `last_failure_count`, and `last_failure_started_at` fields preserve the failure sequence that caused the most recent transition and survive supervisor restarts.

Every failed endpoint probe records the node ID, sanitized URL, curl exit status, HTTP status, connect time, application-connect time, and total time in syslog with the `probe-endpoint-failed` reason. With the SOCKS proxy, `time_connect` ends when curl connects to the local proxy, while `time_appconnect` is cumulative from probe start through proxy negotiation, proxy-side DNS and outbound connection establishment, and—for HTTPS—the remote TLS handshake. It is therefore not labeled as TLS-only duration. URL userinfo through the final raw `@` in the authority, query parameters, fragments, and line-breaking characters are removed from log output so even malformed custom probe credentials cannot leak through diagnostics; `@` characters after the first path slash are preserved. If a raw query or fragment delimiter appears before the final authority `@`, the malformed URL is treated as ambiguous and only its scheme plus `[redacted]` is logged. A confirmed active-node failure is recorded as `node-failed` when another candidate answers the same probes. If no candidate answers, the supervisor checks every IPv4 and IPv6 default route, including every nexthop in an ECMP route, and treats the WAN as available when any route candidate has a usable device whose carrier is not explicitly down. No default route, no route device, or an explicit carrier-down result across every route candidate is recorded as `wan-down`; otherwise the failure is recorded as `probe-endpoint-failed` and follows the configured all-down fallback policy. A family-aware gateway ping is diagnostic only because an unanswered ICMP request does not prove that the WAN is unavailable and must never suppress the all-down fallback.
